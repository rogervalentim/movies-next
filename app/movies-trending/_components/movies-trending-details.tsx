...
import { MoviesTrendingItem } from "./movies-trending-item";
...
export function MoviesTrendingDetails() {
  ...
  const {
    ...
    queryKey: ["moviesTrending", page],
    queryFn: async () => {
      const response = await fetch(
        `https://api.themoviedb.org/3/trending/movie/day?api_key=${apiKey}&language=pt-BR&page=${page}`
      );
      if (!response.ok) {
