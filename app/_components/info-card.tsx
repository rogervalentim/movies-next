...
export function InfoCard({
  ...
  const isPerson =
    media_type === "person" || Boolean(profile_path && !poster_path);
  const typeLabel =
    media_type === "movie"
      ? "Filme"
      : media_type === "tv"
  ...
  return (
    <article className="group">
      <div className="relative aspect-[2/3] overflow-hidden rounded-2xl border border-border bg-card shadow-card transition duration-300 group-hover:-translate-y-1 group-hover:border-primary/50 group-hover:shadow-glow motion-reduce:transform-none">
        {poster_path || profile_path ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 bg-gradient-to-br from-red-100 to-muted text-red-700 dark:from-red-950 dark:to-[#151515] dark:text-red-200">
            {isPerson ? (
