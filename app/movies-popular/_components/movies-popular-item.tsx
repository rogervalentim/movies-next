...
interface MoviesPopularItemProps {
  title: string;
  poster_path: string;
  vote_average: number;
  year: string;
...
export function MoviesPopularItem({
  title,
  poster_path,
  vote_average,
  year
}: MoviesPopularItemProps) {
  return (
    <div className="relative transition-transform duration-300  group-hover:scale-105">
      {poster_path ? (
        <Image
          ...
          quality={100}
          sizes="100vh"
          className="rounded-lg  border border-border  w-full h-64 lg:h-96 "
        />
      ) : (
      ...
      <div className="flex justify-between">
        ...
        <div className="absolute inset-0 flex flex-col justify-center items-start bg-gradient-to-t from-black to-transparent px-4 text-white  rounded-lg">
          <span className="text-lg font-medium line-clamp-2 py-1">{title}</span>
